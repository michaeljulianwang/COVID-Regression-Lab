...
modDat <- stateLevel %>%
  ...
  left_join(
    stateLevel %>% 
      ...
      group_by(sub_region_1) %>%
      summarise(pre_COVID = median(retail_and_recreation_percent_change_from_baseline))
  ) %>%
  left_join(
    COVIDData %>%
...
ggplot(modDat, aes(x = as.numeric(RISKCOV) )) +
  geom_histogram() + ggtitle("Risk COVID Exploration") 
```

```{r, echo=FALSE, message=FALSE}
ggplot(modDat, aes(x = as.numeric(UIMAXAMT) )) +
  geom_histogram() + ggtitle("Max Unemployment Amount") 
```

```{r, echo=FALSE, message=FALSE}
ggplot(modDat, aes(x = pre_COVID )) +
  geom_histogram() + ggtitle("Pre-COVID Movement") 
```

```{r, echo=FALSE, message=FALSE}
mod2 <- lm(y_var ~ length + as.numeric(UIMAXAMT) + pre_COVID + Party + new_case_per_thousand
          , data = modDat)
...
```{r, echo=FALSE, message=FALSE}
mod3 <- lm(y_var ~ length + as.numeric(RISKCOV) + as.numeric(UIMAXAMT) + pre_COVID + 
             MedianAge + as.numeric(POPDEN18) + Party + new_case_per_thousand
          , data = modDat)
