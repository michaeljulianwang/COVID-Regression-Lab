...
modDat <- stateLevel %>%
  ...
  left_join(
    stateLevel %>% 
      ...
      group_by(sub_region_1) %>%
      summarise(pre_COVID = mean(retail_and_recreation_percent_change_from_baseline))
  )
...
ggplot(COVIDout %>% filter(submission_date >= '2020-03-01',
  ...
  geom_vline(xintercept = as.numeric(ymd('2020-06-15')))
```
...
```{r, echo=FALSE, message=FALSE}
mod2 <- lm(y_var ~ length + as.numeric(UIMAXAMT) + pre_COVID
          , data = modDat)
...
mod3 <- lm(y_var ~ length + as.numeric(RISKCOV) + as.numeric(UIMAXAMT) + pre_COVID + 
             MedianAge + as.numeric(POPDEN18) + Party
          , data = modDat)
