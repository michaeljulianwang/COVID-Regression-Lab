...
```{r model 2}
acs <- read.csv('../data/raw/ACSDP1Y2019.DP05-2021-04-04T224220.csv')
acs <- acs[-1,]
aa <- acs[,grep('Estimate',colnames(acs))]
labs <- acs[,1]
med_age <- as.numeric(gsub(',','',aa[18,]))
names(med_age) <- colnames(aa)
# med_age <- aa[grep('Median',labs),]
names(med_age) <- gsub('\\.\\.Estimate','',names(med_age))
names(med_age) <- gsub('\\.',' ',names(med_age))
x <- med_age[ match(modDat$sub_region_1,names(med_age))]
mod2 <- lm(modDat$y_var ~ modDat$stay + as.numeric(x))
coeftest(mod2, vcov = vcovHC(mod2))

plot(as.numeric(x),modDat$y_var) # older males = less mobility, each dot = state


ratio_65 <- as.numeric(gsub(',','',aa[32,]))
names(ratio_65) <- colnames(aa)
# med_age <- aa[grep('Median',labs),]
names(ratio_65) <- gsub('\\.\\.Estimate','',names(ratio_65))
names(ratio_65) <- gsub('\\.',' ',names(ratio_65))
y <- ratio_65[ match(modDat$sub_region_1,names(ratio_65))]
mod3 <- lm(modDat$y_var ~ modDat$stay + as.numeric(y))
coeftest(mod3, vcov = vcovHC(mod3))

plot(as.numeric(y),modDat$y_var) # older males = less mobility, each dot = state

```

### Model 3
```{r model 3}
gov <- read.csv('../data/raw/Govenor Party.csv')
gov$party <- ifelse(gov$Party=='R',0,1)

mod5 <- lm(modDat$y_var ~ modDat$stay + as.numeric(y) + gov$party)
coeftest(mod5, vcov = vcovHC(mod5))

mod4 <- lm(modDat$y_var ~ modDat$stay + as.numeric(x) + as.numeric(y) + gov$party)
coeftest(mod4, vcov = vcovHC(mod4))coeftest(mod4, vcov = vcovHC(mod4))
...
# 
# In your text, comment on both *statistixcal significance and practical significance*. You may want to include statistical tests besides the standard t-tests for regression coefficients.
```
